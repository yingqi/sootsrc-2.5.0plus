 ...
 * </p>
 */
public class ExceptionalUnitGraphPlus extends UnitGraphPlus implements
	 ...
	 */
	public ExceptionalUnitGraphPlus(Body body, ThrowAnalysis throwAnalysis,
	 ...
	 * 
	 */
	public ExceptionalUnitGraphPlus(Body body) {
	 ...
	 */
	protected ExceptionalUnitGraphPlus(Body body, boolean ignoredBogusParameter) {
		super(body);
	}
	...
	protected void initialize(ThrowAnalysis throwAnalysis,
		...
		unitToUnexceptionalSuccs = new HashMap<Unit, List<Unit>>(size * 2 + 1,
				0.7f);
		unitToUnexceptionalPreds = new HashMap<Unit, List<Unit>>(size * 2 + 1,
				0.7f);
		...
		if (body.getTraps().size() == 0) {
			...
			unitToExceptionalSuccs = new HashMap<Unit, List<Unit>>(
					unitToExceptionDests.size() * 2 + 1, 0.7f);
			unitToExceptionalPreds = new HashMap<Unit, List<Unit>>(body
		...
		for (Unit unitHead : heads) {
			if (!unitHead.equals(head)) {
				if (unitToPreds.containsKey(unitHead)) {
					List<Unit> predsList = new ArrayList<Unit>();
					predsList.addAll(unitToPreds.get(unitHead));
					unitToPreds.remove(unitHead);
					unitToPreds.put(unitHead, predsList);
				} else {
					unitToPreds.put(unitHead, headList);
				}
			}
		}
		unitToPreds.put(tail, tails);

		for (Unit unitTail : tails) {
			if (!unitTail.equals(tail)) {
				if (unitToSuccs.containsKey(unitTail)) {
					List<Unit> succsList = new ArrayList<Unit>();
					succsList.addAll(unitToSuccs.get(unitTail));
					unitToSuccs.remove(unitTail);
					unitToSuccs.put(unitTail, succsList);
				} else {
					unitToSuccs.put(unitTail, tailList);
				}
			}
		}
		unitToSuccs.put(head, heads);
	...
	protected Map<Unit, Collection<ExceptionDest>> buildExceptionDests(
		...
		Map<Unit, ThrowableSet> unitToUncaughtThrowables = new HashMap<Unit, ThrowableSet>(
				units.size());
	...
	private Map<Unit, Collection<ExceptionDest>> addDestToMap(
		...
		if (dests == null) {
			if (t == null) {
				...
				if (map == null) {
					map = new HashMap<Unit, Collection<ExceptionDest>>(
							unitChain.size() * 2 + 1);
				}
